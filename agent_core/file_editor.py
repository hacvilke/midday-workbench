...
class FileEditorTool:
    ...
    def write_file(self, path: str, content: str) -> str:
        ...
        return self.write_file_with_metadata(path, content).message

    def write_file_with_metadata(self, path: str, content: str) -> FileWriteResult:
        """Overwrite or create a file and return structured audit metadata."""
        ...
        verb = "Created" if created else "Written"
        message = f"{verb} {len(encoded):,} bytes ({lines} lines) -> {path}"
        return FileWriteResult(path, len(encoded), lines, sha256, created, message)

    def file_metadata(self, path: str, message: str | None = None) -> FileWriteResult:
        """Return structured metadata for an existing file without rewriting it."""
        ...
        return FileWriteResult(
            ...
            sha256,
            False,
            message or f"Inspected {len(encoded):,} bytes ({lines} lines) -> {path}",
        )
